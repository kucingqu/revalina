/*
/^[a-zA-Z][a-zA-Z0-9]{3,32)/gl
/^(?=.*\id)(?=.*[a-z])(?=.*[A-Z])(?=.*[a-zA-Z]).{8,}$/go
*/
...
let isUsernameValid = () => {
/* Username should be contain more than 3 characters. Should begin with alphabetic character Can 
contain numbers */
...
/* Password should be atleast 8 characters long. Should contain atleast 1 number, 1 special symbol

,1 lower case and 1 upper case */
...
"border-color: #fe2e20; background-color: #ffc2c2";
} else {
usernameRef.style.cssText =
...
passwordRef.addEventListener("input", () => {
    ...
    "border-color: #fe2020; background-color: #ffc2c2";
    } else {
    passwordRef.style.cssText
    "border-color: #34bd34; background-color: #c2ffc2";
    }
    ...
    if (!isUsernameValid() || lisPasswordValid()) { 
        //Get the current position of submit btn
    let containerRect = document
    .querySelector(".container")
     .getBoundingClientRect();
    let subsitRect = submitBtn.getBoundingClientRect();
